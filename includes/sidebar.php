...
$rol = $_SESSION['rol'] ?? 'admin'; // 'superadmin' o 'admin'
    ...
    <nav class="flex-1 px-3 py-6">
        <ul class="space-y-2">
            <?php if ($rol === 'superadmin'): ?>
                <li>
                    <a href="<?= $base_url ?>/superadmin/index.php" class="flex items-center gap-2 px-3 py-2 rounded-lg text-blue-700 font-semibold hover:bg-blue-50 hover:text-blue-600 transition">
                        <span>👑</span> Panel Superadmin
                    </a>
                </li>
                <!-- Agrega otros menús de superadmin si necesitas -->
            <?php else: ?>
                <li>
